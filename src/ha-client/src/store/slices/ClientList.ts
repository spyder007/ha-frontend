...
import * as api from "../../api/Unifi/api";
import { AppThunk } from "../store";
import { addMessage, clearMessages } from "./UserMessage";
import { callUnifiApi, checkCommunicationError } from "../../utils/ApiFunctions";
...
export const deleteClient =
  ...
  async (dispatch): Promise<void> => {
    ...
    const result = await callUnifiApi((baseUrl) =>
      // todo: implement paging here
      api.ClientApiFactory(undefined, baseUrl).clientDeleteClient(mac),
    );
...
export const fetchClientList =
  ...
  async (dispatch): Promise<void> => {
    ...
    const result = await callUnifiApi((baseUrl) =>
      api.ClientApiFactory(undefined, baseUrl).clientGet(),
    );
    const error = checkCommunicationError(result);
    if (error) {
      dispatch(
